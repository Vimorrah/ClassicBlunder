...
/SecretInformation/HeavenlyRestriction/proc/applySecretVariable(mob/p, list/restr, improv)
	...
	switch(improv)
		if("Dragon Clash")
			p.passive_handler.Increase("Technique Mastery", min(boon_value, 5))
			p.passive_handler.Increase("Attack Speed", min(boon_value, 5))
		if("Grab")
			p.passive_handler.Increase("Grippy", min(boon_value, 5))
			p.passive_handler.Increase("Muscle Power", min(boon_value, 5))
		if("Reverse Dash")
		...
		if("Dragon Dash")
			...
			p.passive_handler.Increase("Skimming", min(boon_value, 5))
		if("Strength")
			p.passive_handler.Increase("Momentum", min(boon_value, 5))
			p.passive_handler.Increase("PhysPleroma", min(boon_value * 0.5, 2.5))
		if("Force")
			p.passive_handler.Increase("Spirit Hand", min(boon_value, 5))
			p.passive_handler.Increase("Spirit Sword", min(boon_value, 5))
			p.passive_handler.Increase("Spirit Flow", min(boon_value, 5))
		if("Endurance")
			p.passive_handler.Increase("Juggernaut", min(boon_value, 5))
			p.passive_handler.Increase("GiantForm", 1)
		if("Offense")
			p.passive_handler.Increase("Pressure", min(boon_value, 5))
			p.passive_handler.Increase("ComboMaster", 1)
		if("Defense")
			p.passive_handler.Increase("Fluid Form", min(boon_value, 5))
			p.passive_handler.Increase("Like Water", min(boon_value, 5))
		if("Speed")
			p.passive_handler.Increase("Speed Force", 2)
			p.passive_handler.Increase("Pursuer", min(boon_value, 5))
		if("Anger")
			p.passive_handler.Increase("Calm Anger", 1)
			p.passive_handler.Increase("Wrathful Tenacity", 0.2)
		if("Power Control")
			p.passive_handler.Increase("Energy Generation", min(boon_value * 1.5, 5))
			p.passive_handler.Increase("Soft Style", min(boon_value * 0.5, 2.5))
		if("Power")
			p.passive_handler.Increase("Buff Mastery", min(boon_value, 5))
